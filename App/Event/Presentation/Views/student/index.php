        ...
        <div class="flex flex-col lg:flex-row gap-3">
            ...
            <div class="relative flex-1">
                <i data-lucide="search"
                    class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 peer-focus:text-blue-500 transition-all duration-300 peer-focus:scale-110"></i>
                <input type="text" name="search" value="<?= htmlspecialchars($_GET['search'] ?? '') ?>"
                    placeholder="Search events..." onkeydown="if(event.key==='Enter'){this.form.submit()}"
                    class="peer w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200/80 bg-white/50 text-sm placeholder:text-slate-400 focus:border-blue-400 focus:ring-4 focus:ring-blue-100/60 focus:bg-white outline-none transition-all duration-300 shadow-sm focus:shadow-lg hover:border-blue-200">
            </div>
            ...
            <div class="relative">
                ...
                <select name="status" onchange="this.form.submit()"
                    class="appearance-none w-full lg:w-56 pl-11 pr-9 py-3 rounded-xl border border-slate-200/80 bg-white/50 text-sm cursor-pointer focus:border-blue-400 focus:ring-4 focus:ring-blue-100/60 focus:bg-white outline-none transition-all duration-300 hover:border-blue-300 shadow-sm hover:shadow-md">
                    <option value="">All Events</option>
                    <option value="published" <?= ($_GET['status'] ?? '') == 'published' ? 'selected' : '' ?>>Upcoming
                        Events</option>
                    <option value="completed" <?= ($_GET['status'] ?? '') == 'completed' ? 'selected' : '' ?>>Past
                        Events</option>
                </select>
