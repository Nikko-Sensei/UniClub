<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Students Management</h1>
            <p class="text-slate-500">Overview and administration of all registered university students.</p>
        </div>
        ...
        <form method="GET" action="<?= BASE_URL ?>/admin/users" class="flex flex-wrap items-end gap-3">
            ...
            <div class="relative flex-1 min-w-[200px]">
                <i data-lucide="search"
                    class="absolute left-3.5 top-0 h-full flex items-center text-slate-400 w-4 h-4"></i>
                <input type="text" name="search" placeholder="Search by student name, ID or email..."
                    value="<?= htmlspecialchars($_GET['search'] ?? '') ?>"
                    class="w-full pl-9 pr-4 py-2.5 border border-slate-200/80 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition text-sm bg-white/50 backdrop-blur-sm hover:border-blue-200" />
            </div>

            <!-- Department filter -->
            <div class="flex-1 min-w-[140px]">
                <select name="department_id" onchange="this.form.submit()"
                    class="w-full border border-slate-200/80 rounded-lg px-3 py-2.5 text-sm bg-white/50 backdrop-blur-sm text-slate-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30 transition appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2212%22%20height%3D%2212%22%20viewBox%3D%220%200%2012%2012%22%3E%3Cpath%20fill%3D%22%2364758b%22%20d%3D%22M6%208L1%203h10z%22%2F%3E%3C%2Fsvg%3E')] bg-[right_12px_center] bg-no-repeat bg-[length:12px] hover:border-blue-200">
    ...
    <div
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="8" class="px-5 py-10 text-center text-slate-400">
                            <i data-lucide="users" class="w-10 h-10 mx-auto text-slate-300 mb-3 block"></i>
                            No users found.
                        </td>
                    </tr>
                    <?php else: ?>
                    <?php foreach ($users as $user): ?>
                    <tr class="border-b border-slate-100/60 hover:bg-slate-50/30 transition-colors">
                        <!-- Profile -->
                        <td class="px-5 py-3.5">
                            <div class="flex items-center">
                                <div class="w-9 h-9 rounded-full overflow-hidden flex items-center justify-center 
                                    bg-gradient-to-br from-blue-100 to-blue-200 
                                    text-blue-700 font-semibold text-xs shadow-sm">
                        ...
                        <td class="px-5 py-3.5">
                            <span
                                class="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-50 text-indigo-700 text-xs font-medium rounded-full border border-indigo-100/50">
        ...
        <div
            ...
            <div class="flex items-center gap-2">
                ...
                <?php if ($i == $current): ?>
                <span
                    class="w-8 h-8 bg-blue-600 text-white rounded-lg text-xs font-medium flex items-center justify-center shadow-sm">
                    <?= $i ?>
                </span>
                <?php else: ?>
